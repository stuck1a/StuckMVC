 ...
 * Smarty plugin
 * ----------------------------------------------------------------------------------
 * File:     function.getStylesPath.php
 * Type:     function
 * Name:     getStylesPath
 * Purpose:  returns the absolute path to the registered directory for css files
 * ----------------------------------------------------------------------------------
 */

function smarty_function_getStylesPath(array $params, Smarty_Internal_Template $template): string {
  // fetch document root path from server variables
  $result = ($_SERVER['REQUEST_SCHEME']?:'https') . '://' . $_SERVER['SERVER_NAME'];
  // add mapping of the css directory plus trailing slash for better usability of plugin
  return $result . \Stuck1A\StuckMVC\Core\Mapper::getInstance()->getDir('css', 1) . '/';
}
